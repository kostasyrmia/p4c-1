...
parser ParserImpl(packet_in packet, out headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    data_t hdr2_0_h1;
    data_t p_local_hdr2_h1;
    state start {
        hdr2_0_h1.setInvalid();
    ...
    state PacketParser_local {
        p_local_hdr2_h1.f = 8w10;
        p_local_hdr2_h1.setValid();
        transition PacketParser_parse_h2;
    }
    state PacketParser_local_0 {
        p_local_hdr2_h1.f = 8w11;
        p_local_hdr2_h1.setValid();
        transition PacketParser_parse_h2;
    }
    state PacketParser_local_1 {
        p_local_hdr2_h1.f = 8w12;
        p_local_hdr2_h1.setValid();
        transition PacketParser_parse_h2;
    }
    state PacketParser_parse_h2 {
        hdr2_0_h1.f = p_local_hdr2_h1.f;
        hdr2_0_h1.setValid();
...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    @hidden action parser_subparser_v1model158() {
        standard_metadata.egress_port = 9w4;
    }
    @hidden action parser_subparser_v1model160() {
        standard_metadata.egress_port = 9w3;
    }
    @hidden action parser_subparser_v1model162() {
        standard_metadata.egress_port = 9w2;
    }
    @hidden action parser_subparser_v1model164() {
        standard_metadata.egress_port = 9w10;
    }
    @hidden table tbl_parser_subparser_v1model158 {
        actions = {
            parser_subparser_v1model158();
        }
        const default_action = parser_subparser_v1model158();
    }
    @hidden table tbl_parser_subparser_v1model160 {
        actions = {
            parser_subparser_v1model160();
        }
        const default_action = parser_subparser_v1model160();
    }
    @hidden table tbl_parser_subparser_v1model162 {
        actions = {
            parser_subparser_v1model162();
        }
        const default_action = parser_subparser_v1model162();
    }
    @hidden table tbl_parser_subparser_v1model164 {
        actions = {
            parser_subparser_v1model164();
        }
        const default_action = parser_subparser_v1model164();
    }
    apply {
        if (hdr.h4.isValid()) {
            tbl_parser_subparser_v1model158.apply();
        } else if (hdr.h3.isValid()) {
            tbl_parser_subparser_v1model160.apply();
        } else if (hdr.h2.isValid()) {
            tbl_parser_subparser_v1model162.apply();
        } else {
            tbl_parser_subparser_v1model164.apply();
        }
    }
}
